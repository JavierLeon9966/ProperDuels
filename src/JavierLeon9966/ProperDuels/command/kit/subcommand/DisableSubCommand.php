...
class DisableSubCommand extends BaseSubCommand{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args): void{
		Await::f2c(function() use($sender, $args): Generator{
			...
			$status = yield from $this->kitManager->setEnabled($args['kit'], false);
			...
			$sender->sendMessage(match($status){
				KitUpdateStatus::SUCCESS => "Disabled kit '$args[kit]' successfully",
				KitUpdateStatus::NOT_FOUND => TextFormat::RED . "A kit with the name '$args[kit]' does not exist",
				KitUpdateStatus::NO_CHANGES => TextFormat::RED . "The kit '$args[kit]' is already disabled"
			});
		});
	}
	...
	public function prepare(): void{
		...
		$this->setPermission('properduels.command.kit.disable');
